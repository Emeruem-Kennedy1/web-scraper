...
export class Scrapper {
  ...
  private async handleError(error: any, page: Page): Promise<Page> {
    ...
    if (error.name === "TimeoutError") {
      ...
      console.log(
        "Creating new page due to connection error or destroyed context"
      );
      await page.close();
  ...
  public async run<T>(
    customFunction: ScrapperFunction<T>,
    args: any[],
    retries = 3
  ): Promise<T> {
  ...
  private async runSingleTask<T>(
    task: { func: ScrapperFunction<T>; args: any[] },
    retries = 3
  ): Promise<T> {
    const page = await this.getPageFromPool();
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        currentPage = (await this.handleError(
          error,
          currentPage
        )) as PageWithStatus;
        ...
        if (this.pagePool.indexOf(currentPage as PageWithStatus) === -1) {
          this.pagePool[this.pagePool.indexOf(page as PageWithStatus)] =
            currentPage as PageWithStatus;
        }
      }
    }
    throw new Error("Unexpected error: All retries failed");
