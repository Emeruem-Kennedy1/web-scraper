...
async function saveArtistsToDatabase(artists: string[]) {
  ...
  const newArtists = uniqueArtists.filter(name => !existingArtistNames.has(name));
  ...
  */
  ...
  console.log(`Processed ${uniqueArtists.length} artists. Created ${newArtists.length} new artists.`);
}
...
async function getLastSuccessfulPage(category: string, progress: Record<string, number>): Promise<number> {
...
async function scrapeCategoriesPageCounts() {
  ...
  try {
    const tasks = categoriesSet.map(category => ({
      ...
      args: [category, () => console.log(`Finished scraping category ${category}`)]
    }));
...
async function scrapeArtists(jsonFilePath: string) {
  ...
  try {
    for (const { category, pageCount } of categoryPageCounts) {
      const lastSuccessfulPage = await getLastSuccessfulPage(category, progress);
      console.log(`Last successful page for category ${category}: ${lastSuccessfulPage}`);
      const startPage = lastSuccessfulPage + 1;
      logger.info(`Scraping category ${category} starting from page ${startPage}`);

      for (let page = startPage; page <= pageCount; page += 5) { // Process in batches of 5
        const endPage = Math.min(page + 4, pageCount);
        const tasks = [];
        for (let p = page; p <= endPage; p++) {
          tasks.push({
            func: getArtistsOnPage,
            args: [category, p, () => logger.info(`Finished scraping category ${category}, page ${p}`)]
          });
        }
        ...
        for (let i = 0; i < results.length; i++) {
          ...
          if ('error' in result) {
            logger.error(`Error scraping category ${category}, page ${currentPage}: ${result.error}`);
